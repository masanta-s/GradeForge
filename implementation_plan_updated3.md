...
| 6 | **SSIM on hand-drawn diagrams** mostly measures stroke/position noise | Phase 4 | Weight label matching + VLM judgement; keep SSIM low-weight or drop it |
| 7 | **"≥20 corrections from the same writer"** needs a `writer_id`; a 4-sample held-out set gives a noisy CER | Phase 7 | Add `writer_id` to `ocr_corrections`; use k-fold CER or a larger threshold |
