...
> - **Kaggle is driven by its API** (`kaggle` 2.2.4, token in `.env`): GradeForge pushes a private notebook, polls its status, tails its log and downloads the adapter. Measured 2026-09-12 with a probe notebook: `machine_shape: NvidiaTeslaT4` yields 2x Tesla T4 (14.6 GiB each), internet on, 31 GiB RAM, ~1 TB scratch in /tmp, 19.5 GiB output space; `torch.cuda.is_bf16_supported()` is True on T4 but only emulated, so fp16 is used there.
