...
|---|---|---|---|---|---|
| **Gemma 3 12B** | 12B | ~7 GB | ~7.5 GB (tight) | ✅ Yes | 🟢 If HF weights resolve |
| **Qwen 3 8B** | 8B | ~5 GB | ~6 GB | ❌ | 🟢 If HF weights resolve |
...
| **Gemma 3 4B** | 4B | ~2.5 GB | ~3.5 GB | ✅ Yes | 🟢 If HF weights resolve |

> [!NOTE]
> **RTX 4060 ceiling:** QLoRA tops out around 12–14B locally. Anything larger routes to the Colab/Kaggle export path.

#### 📸 For OCR (Reading Handwritten Text)

| Model | Type | VRAM | Fine-tunable |
|---|---|---|---|
| **TrOCR** (`trocr-base-handwritten`) | Dedicated OCR | ~350 MB | ✅ Always (LoRA, fixed component) |
...
class CapabilityProber:
    ...
    def probe(self, model_name: str) -> ProbeReport:
        ...
        4. Vision (optional) — send a test image, check response.
           Failure just routes OCR to TrOCR.
...
│                                                             │
│  Current model: Gemma 3 12B (Q4) — 🟢 Trainable            │
│                                                             │
...
│    └ ~45 min • Free • Private                                │
│    └ VRAM: 7.2/8.0 GB ⚠️ tight but fits                     │
│    └ Disk: needs 52 GB (you have 180 GB free ✅)             │
│                                                             │
...
> *"I built PaperMind — a full-stack, provider-agnostic AI exam evaluator with a React frontend and FastAPI backend. There's no model whitelist: the system reads GGUF metadata from Ollama, resolves the original HuggingFace training source by searching the Hub and verifying parameter counts, and gates trainability on whether the local transformers library supports the architecture — so a model released next month works automatically, and the error message names the pip upgrade that unlocks it. Student data never leaves the machine — model metadata comes from Ollama and HuggingFace, but question papers, scores, corrections, and dispute logs are processed and stored 100 % locally, even when cloud LLMs are optionally used for inference. The system learns through four independent mechanisms: dynamic few-shot retrieval from a corrections database that works from the very first correction on any model including Claude and GPT; per-model isotonic score calibration that compensates for systematic grading bias; TrOCR LoRA fine-tuning with CER-gated checkpoint promotion; and optional LLM QLoRA via Unsloth with the full GGUF reconversion pipeline. Three of these mechanisms work on every model — the fourth is offered only when the HuggingFace source resolves and the architecture is supported. Before any model enters the grading pipeline, a capability probe tests JSON adherence, instruction following, and rubric reasoning, with four defence layers handling malformed output. The grading engine uses a strictness slider controlling a power-curve scoring function, and the system handles handwritten text and hand-drawn diagrams with teacher-configurable weightage and a full dispute resolution system with audit trails."*
...
- Computer Vision & OCR (TrOCR, handwritten text recognition, CER-gated checkpoints)
